...
class PerformanceTest extends Component {
    ...
    renderLangOptions () {
        return this.props._dockerLangs.map((lang, key) => (
            <MenuItem key={key} value={lang._id} primaryText={lang.title} />
        ));
    }
    ...
    deleteIcon (cases) {
        return (
            <IconButton
                ...
                onTouchTap={this.removeCase.bind(this, cases)}
            >
                <DeleteIcon />
            </IconButton>
        );
    ...
    renderTestCases () {
        return this.props._testCases.map((cases, key) => (
            <ListItem
                key={key}
                primaryText={this.getLanguageTitle(cases.langType)}
                secondaryText={cases.langType}
                rightIconButton={this.deleteIcon(cases)}
                onTouchTap={this.caseDialogOpen.bind(this, cases)}
                ...
                Meteor.call('docker.performanceTest', {
                    ...
                    if (resolveCount === sentCount) {
                        ...
                        this.setState({
                            result:
                                'It takes '
                                + String(Math.round((totalTime*1000))/1000)
                                + ' seconds to execute ' + String(sentCount)
                                + ' Submission'
                        });
                    }
                });
    ...
    runSingleTest() {
        ...
        var choosenCase = testCases[
            (Math.floor((Math.random() * 100) + 1) % testCasesCount)
        ...
        Meteor.call('docker.performanceTest', {
            ...
            if (this.state.repeatMyself && !err) {
                ...
                // If we reach maximum concurrent user, stall for 0.5s otherwise
                // it gets too brutal for logger
                setTimeout(()=> {
                    this.runSingleTest();
                }, 1000);
            }
    ...
    keepTesting (isRepeat) {
        ...
        if (!isRepeat) { //Submit test case periodically
            ...
            keepTestingInterval = setInterval(
                this.runSingleTest.bind(this), period * 1000);
    ...
    render () {
        const actions = [
            <FlatButton
                label="Cancel"
                secondary={true}
                onTouchTap={this.caseDialogOpen.bind(this, false)}
            />,
            <FlatButton
                label="Update"
                primary={true}
                onTouchTap={this.updateCase.bind(this, null)}
            />
        ];
        return (
            <MuiThemeProvider muiTheme={getMuiTheme(baseTheme)}>
                <div>
                    ...
                    <RaisedButton
                        label="Add Test Case"
                        primary={true}
                        onTouchTap={this.addCase.bind(this)}
                    />
                    <RaisedButton
                    ...
                    <div>
                        <RaisedButton
                            label="Keep Sending Random Test Case Periodically"
                            primary={true}
                            onTouchTap={this.keepTesting.bind(this, false)}
                        />
                        <TextField
                            type="text"
                            value={this.state.period}
                            floatingLabelText="Period (seconds)"
                    ...
                    <div>
                        <RaisedButton
                            label="Sending Random Test Case after one another"
                            primary={true}
                            onTouchTap={this.keepTesting.bind(this, true)}
                        />
                    </div>
                    {
                        this.state.selectCase ?
                            <Dialog
                                ...
                                contentStyle={{width: '90%', maxWidth: '100%'}}
                                open={this.state.caseDialog}
                                onRequestClose={
                                    this.caseDialogOpen.bind(this, false)
                                }
                                autoDetectWindowHeight={true}
                            >
                                <SelectField
                                    value={this.state.selectCase.langType}
                                    onChange={this.updateLangType.bind(this)}
                                ...
                                <div>
                                    <TextField
                                        type="text"
                                        value={this.state.selectCase.testInput}
                                    ...
                                    <TextField
                                        type="text"
                                        value={this.state.selectCase.testScript}
                                        floatingLabelText="Testing Script"
                                        onChange={
                                            this
                                            .updateCaseState
                                            .bind(this, 'testScript')
                                        }
                                        multiLine={true}
                                        style={{width: '100%'}}
                                    />
                    ...
                    }
                    {
                        ...
                        <Dialog
                            ...
                            onRequestClose={
                                this.resultDialogOpen.bind(this, false)
                            }
                            autoDetectWindowHeight={true}
                        >
                            <LinearProgress
                                mode="determinate"
                                value={this.state.testCaseResolved}
                                max={this.state.testCaseSent}
                            />
                            {
                                this.state.testCaseResolved
                            }/{
                                this.state.testCaseSent
                            }
                            <div>
                    ...
                    }
                    {
                        this.state.keepTestingDialog ?
                            <Dialog
                                ...
                                autoScrollBodyContent={true}
                                contentStyle={{width: '90%', maxWidth: '100%'}}
                                open={this.state.keepTestingDialog}
                                onRequestClose={
                                    this.keepTestingDialogOpen.bind(this, false)
                                }
                                autoDetectWindowHeight={true}
                            >
                                <div>time: {this.state.keepTestingExeTIme}</div>
                                {
                                    this.state.testErr ?
                                        <div style={{color: 'red'}}>
                                            <h3>{this.state.testErr.reason}</h3>
                                        </div> :
                                        <div>
                                            Result:
...
PerformanceTest.childContextTypes = {
